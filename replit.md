...
**November 26, 2025 (Latest)**: Fixed duplicate Slack notification bug caused by simultaneous workflow runs. Implemented atomic workflow lock mechanism using `workflow_locks` table with INSERT...ON CONFLICT for idempotent execution. Step 1 now acquires a lock before proceeding, blocking duplicate runs. Also added fallback check for recently completed reports (within 30 minutes). Locks auto-expire after 30 minutes for cleanup. This prevents multiple Slack notifications when workflow is triggered multiple times rapidly. Note: If a workflow fails before Step 13, the lock persists until the 30-minute expiry - this is a known limitation that allows eventual recovery.

**November 26, 2025**: Fixed critical data flow bug where Steps 3-4 overwrote Step 2's saved data. Added 3 new targeted DB methods (`updateCompetitorSources`, `updateIndustrySources`, `updateReviewsSources`) that only update their respective columns instead of overwriting all data. Introduced lightweight `workflowContextSchema` to pass only essential metadata (dates, runId, labels) between workflow steps, with heavy scraped content saved directly to DB. Restored 5 missing thematic web search queries in Step 5 (broad pulse, M&A, partnerships, competitive positioning, product themes). Total Step 5 now executes 10 searches (5 thematic + 5 per-competitor).

**November 25, 2025**: Migrated system to user-specified 5-competitor DAP market (WalkMe, Whatfix, Pendo, Appcues, Apty). Updated all constants, workflow logic, metric extraction, and database sources with 23 competitor-specific URLs including G2 reviews, Gartner reviews/likes-dislikes, case studies, newsrooms, analyst reports, and changelogs. New category system includes: news, case_studies, analyst_reports, changelog, g2_reviews, gartner_reviews, gartner_likes_dislikes with appropriate timeFilters (7days, 1month, current_month, quarter).
